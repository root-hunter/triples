...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let max_time = times_euclide.iter()
    ...
    chart.draw_series(LineSeries::new(
        LIMITS.iter().zip(times_euclide.iter()).map(|(&x, &y)| (x, y)),
        &RED,
    ))?.label("Euclide").legend(|(x,y)| PathElement::new(vec![(x,y),(x+20,y)], &RED));

    // Berggren line
    chart.draw_series(LineSeries::new(
        LIMITS.iter().zip(times_berggren.iter()).map(|(&x, &y)| (x, y)),
        &BLUE,
    ))?.label("Berggren").legend(|(x,y)| PathElement::new(vec![(x,y),(x+20,y)], &BLUE));
